...
Tick.prototype = {
	...
	render: function (index, old, opacity) {
		...
		if (gridLineWidth) {
			...
			if (options.thanxTweakGridLine) gridLinePath = options.thanxTweakGridLine(gridLinePath, index);
...
Pointer.prototype = {
		...
		if (tooltip && pointer.options.tooltip.shared && !(hoverSeries && hoverSeries.noSharedTooltip)) {
			...
			for (j = 0; j < i; j++) {
				if (series[j].visible &&
					...
					// THANX TICKET: https://github.com/highslide-software/highcharts.com/issues/2350
	...
	onDocumentMouseMove: function (e) {
		...
		// THANX HACK, ticket: https://thanxapp.atlassian.net/browse/TA-4439
	...
	getColumnMetrics: function () {
		...
		var categoryWidth = mathMin(
			...
			pointXOffset = pointPadding + (groupPadding + colIndex *
				...
				(reversedXAxis ? -1 : 1);
			// THANX
			if (options.thanxTweakPointSize){
				var tweaks = options.thanxTweakPointSize(pointWidth, pointPadding);
...
Scroller.prototype = {
	...
	drawHandle: function (x, index) {
		...
		if (!scroller.rendered) {
			...
			tempElem = renderer.rect(-4.5, 0, 9, 16, 3, 1)
				.attr(attr)
				.add(handles[index]);
			if (handlesOptions.thanxTweakGrip) handlesOptions.thanxTweakGrip('grip', tempElem, index);
			...
			tempElem = renderer.path([
				...
				.add(handles[index]);
			if (handlesOptions.thanxTweakGrip) handlesOptions.thanxTweakGrip('rifle', tempElem, index);
			elementsToDestroy.push(tempElem);
		}
	...
	init: function () {
		...
		if (scroller.navigatorEnabled) {
			...
			scroller.xAxis = xAxis = new Axis(chart, (navigatorOptions.thanxTweakNavigatorOptions || function(o){return o;})(merge({
